...
from utils.Time_NLP.RangeTimeEnum import RangeTimeEnum

try:
    from utils.Time_NLP.LunarSolarConverter.LunarSolarConverter import *
except:
    from utils.Time_NLP.LunarSolarConverter import *
...
class TimeUnit:
    def __init__(self, exp_time, normalizer, contextTp):
        self._noyear = False
        self.exp_time = exp_time    # 正则匹配的时间字符串
        # print(exp_time)
        ...
        self.han2num = {'一':1, '二':2, '三':3, '四':4, '五':5, '六':6, '七':7, '天':7, '日':7, '八':8, '九':9, '十':10}
    ...
    def norm_setmonth_fuzzyday(self):
        ...
        rule = r"(?<![0-9])" + month + r"[-\./月]" + day  # xxxx.mm.dd
    ...
    def norm_sethour(self):
        ...
        # rule = u"(?<!(周|星期))([0-2一二三四五六七八九十]?[0-9一二三四五六七八九十])(?=(点|时))"  # '今晚八九点' 需修改regex.txt和reg.pkl
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            try:
                ...
                self.han2num = {'一':1, '二':2, '三':3, '四':4, '五':5, '六':6, '七':7, '天':7, '日':7, '八':8, '九':9, '十':10}
                hour = self.han2num.get(match.group()[0], 7)
            # print(hour)
    ...
    def norm_setSpanRelated(self):
        ...
        if match is not None:
            ...
            self.tp.tunit[3] = int(cur.datetime.hour)
            self.tp.tunit[4] = int(cur.datetime.minute)
        ...
        if match is not None:
            # t_now = arrow.utcnow()
            # t_now = t_now.shift(minutes=30)
            cur = cur.shift(hours=0.5)
            self.tp.tunit[3] = int(cur.datetime.hour)
            self.tp.tunit[4] = int(cur.datetime.minute)
        ...
        if match is not None:
            if self.tp.tunit[1] == -1:
                self.tp.tunit[1] = cur.datetime.month
                
            if self.tp.tunit[1] in [1,3,5,7,8,10,12]:
                self.tp.tunit[2] = 31  
            elif self.tp.tunit[1] in [2]:
                self.tp.tunit[2] = 28 
            else:
                self.tp.tunit[2] = 30
    ...
    def norm_setCurRelated(self):
        ...
        # 这一块还是用了断言表达式
        cur = arrow.get(self.normalizer.timeBase, "YYYY-M-D-H-m-s")
        # print(self.normalizer.timeBase)   # 当前时间不应该是固定的？是动态变化的！
        flag = [False, False, False]
        self.han2num = {'一':1, '二':2, '三':3, '四':4, '五':5, '六':6, '七':7, '天':7, '日':7, '八':8, '九':9, '十':10}
        ...
        if match is not None:
            ...
            try:
                ...
                week = self.han2num.get(match.group(), 1)
            week -= 1
        ...
        rule = u"(?<=((?<!(上|下|个|[0-9]))(周|星期)))[1-7一二三四五六七日天]"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            flag[2] = True
    ...
    def preferFuture(self, checkTimeIndex):
        ...
        cur = arrow.get(self.normalizer.timeBase, "YYYY-M-D-H-m-s")  # 当前时间不应该是固定的？是动态变化的！
