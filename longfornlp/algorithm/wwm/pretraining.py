...
model_saved_path = './models_new/bert_model.ckpt'
corpus_paths = [
...
checkpoint_path = '../../../corpus/chinese_L-12_H-768_A-12/bert_model.ckpt'  # 如果从零训练，就设为None
...
class ModelCheckpoint(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        bert.save_weights_as_checkpoint(filename=model_saved_path+str(epoch)+'.ckpt')

# 保存模型
checkpoint = ModelCheckpoint()
#只保存最佳model
#checkpoint = keras.callbacks.ModelCheckpoint(model_saved_path,monitor='mlm_acc',verbose=0,save_best_only=True, save_weights_only=False, mode='auto', period=1)
